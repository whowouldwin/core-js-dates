...
function getWeekNumberByDate(date) {
  ...
  const diffInDays = Math.floor((utcDate - yearStart) / (1000 * 60 * 60 * 24));
...
function getNextFridayThe13th(date) {
  ...
  let result;
  while (!found) {
    const checkedDate = new Date(year, month, 13);
    if (checkedDate.getDay() === 5) {
      result = checkedDate;
      found = true;
    } else {
      month += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatCurrentDate(currentDate));
